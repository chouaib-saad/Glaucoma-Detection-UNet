...
class OD_localization():
  def __init__(self):
    # loading template using absolute paths
    templates_dir = os.path.join(SCRIPT_DIR, 'Templates')
    image_R = cv2.imread(os.path.join(templates_dir, 'ROItemplateRed.png'), 0)
    image_G = cv2.imread(os.path.join(templates_dir, 'ROItemplateGreen.png'), 0)
    image_B = cv2.imread(os.path.join(templates_dir, 'ROItemplateBlue.png'), 0)
